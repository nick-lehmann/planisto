...
export class CourseTable {
	...
	parseExtentColumn(cell: HTMLElement): Extent {
		...
		const [lecture, exercise, practical] = extentCell.textContent
			.split('/')
	...
	parse(): Course[] {
		return Object.values(this.tableElement.querySelector('tbody').querySelectorAll('tr')).map(
			(row) => {
				...
				const course = new Course({ name, extent, teachers, languages, institute });
